...
public class insertTable {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try(PrintWriter out = response.getWriter()){
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet - Insert Table</title>");            
            out.println("</head>");
            out.println("<body>");
            try(Connection conn = DriverManager.getConnection(url, uname, pass)){
            
                try(Statement s = conn.createStatement()){
                    s.executeUpdate("INSERT INTO student(UID, NAME, COURSE, SECTION) VALUES"
                            +"('23MCI10011','SHIVAM','MAM','MAM-1A'),"
                            +"('23MCI10086','SATYAM','MAM','MAM-1A');");
